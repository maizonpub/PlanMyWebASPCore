<?php
/** 
 * TG Theme-option Menubar.
 */
function tg_theme_tab_menu() {
    global $tg_options;		
		 $counter=0;
		 $menu = "";
...
/**
 * TG Theme-option Field.
 */	
function tg_theme_tab_page() {
	global $tg_options;				
	...
	foreach ( $tg_options as $value ) {				        
		...
		case "text" :	
				...
				$output .= '<label> '.esc_html($value['name']).' </label> <p> '.$value['description'].' </p>';
				$output .= '</div>';
				$output .= '<div class="col-md-7">';
				$output .= '<input class="form-control" type="text" name=" tg_options'.esc_attr('['.$value['id'].']').'" 
							value = "'.esc_attr( $val ).'" /> ';
				$output .= '</div>';			
				$output .= '</div>';
		break;
		...
		case "textarea" :
				...
				$output .= '<textarea class="form-control" name="tg_options'.esc_attr('['.$value['id'].']' ).'"/>'.esc_attr($val).'</textarea>';
				$output .= '</div>';			
				$output .= '</div>';
		break;
		case "hidetextarea" :
		...
		case "logo" :
				...
				$output .= '</div>';
				$output .= '<div class="col-md-7">';								
				$output .= '<span class="upload '.strtolower(str_replace(" ","-",$value['name'])).'">';
				$output .= '<input type="hidden" id="tg_logo" class="regular-text form-control text-upload" 
				name="tg_options'.esc_attr('['.$value['id'].']' ).'"  value = "'.esc_attr( $val ).'"  /><br />';
				if( esc_url( $val ) != '' ) {
		...
		case "radio" : 
				...
				foreach ($value['options'] as $key => $option) {
					...
					if( $val == $key ) {
						...
						$output .= '<div class="radio_layout"><input type="radio" name="tg_options' . esc_attr( $name ) . '"  
		...
		case "select" : 
			...
			$output .= '<select class="form-control" name="tg_options'.esc_attr('['.$value['id'].']').'" >';
			foreach ($value['options'] as $key => $option ) {
				if( $key == $val ) {
		...
		case "multiselect" : 
			...
			{
				$output .= '<select class="form-control" multiple name="tg_options'.esc_attr('['.$value['id'].']').'[]" >';			
			}	
		...
		case "typography" :
			...
			$output .= '<input class="form-control" type="text" name="tg_options' . esc_attr( '[' . $value['id_1'] . ']' ) . ' " 
						value = "'. $val_1 .'" /> 
			...
			$output .= '<div class="input-group custom-search-form">';
			$output .= '<input class="form-control" type="text" name="tg_options' . esc_attr( '[' . $value['id_2'] . ']' ) . ' " 
			...
			$output .= '<input class="color-field" type="text" name="tg_options' . esc_attr('['.$value['id_3'].']').' " 
		...
		case "color" :
				...
				$output .= '</div>';
				$output .= '<div class="col-md-7"> ';
				$output .= '<input class="color-field" type="text" name="tg_options'.esc_attr('['.$value['id'].']').'" 
							value = "'.esc_attr( $val ).'" /> ';
				$output .= '</div>';
				$output .= '</div>';														
		break;
		...
		case "editor" :
				...
				$output .= '<div class="col-md-7"> ';								
				echo $output;
					$textarea_name = 'tg_options'.esc_attr('['.$value['id'].']').'';
