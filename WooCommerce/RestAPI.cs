...
namespace WooCommerceNET
{
    public class RestAPI
    {
            ...
            try
            {
                if (wc_url.StartsWith("https", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    else
                    {
                        ...
                        string link = wc_url + GetOAuthEndPoint(method.ToString(), endpoint, parms);
                        httpWebRequest = (HttpWebRequest)WebRequest.Create(link);
                    }
                }
                else
                {
        ...
        {
           var rest = await SendHttpClientRequest(endpoint, RequestMethod.GET, string.Empty, parms).ConfigureAwait(false);
            return rest;
        }

        public async Task<string> PostRestful(string endpoint, object jsonObject, Dictionary<string, string> parms = null)
        {
            var r = await SendHttpClientRequest(endpoint, RequestMethod.POST, jsonObject, parms).ConfigureAwait(false);
            return r;
        }
